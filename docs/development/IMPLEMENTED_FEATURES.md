...
- 普通会员 30 秒 per-user cooldown、同一 ticker 全频道 60 秒 cooldown；Manager / Owner
  ...
  single-flight 继续保护 Massive；production runtime 已获 Owner 批准并切换为 `MEMBER_LOUNGE`。
...
- Mentor 和关联订单下拉。
- LEAPS 操作后持仓改为 optional；新入场未选择时默认 1/8，已有订单更新未选择时保留当前
  持仓，关闭类操作自动归零。该规则同时覆盖 Review 缺失字段、批准和 Publication 防御校验。
...
- `📍・spy-0dte` receives one new card per five-minute slot from 09:35 ET through the actual close;
  holidays/half days, idempotency, no-backfill and fail-closed behavior are enforced.
