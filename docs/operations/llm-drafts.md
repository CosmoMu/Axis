...
`LLM_API_KEY` 为空时 Bot 仍会正常保存信号，来源状态保持 `RECEIVED`，并提示已
安全排队。填好 Key 并重启后，worker 会从最早的待处理信号开始解析。
...
3. 通过 OpenAI Responses API 发送文字和图片，要求严格 JSON Schema 输出。
...
1. 在 `.env` 直接填入有效 `LLM_API_KEY`，不要经由聊天或 shell 命令传递。
