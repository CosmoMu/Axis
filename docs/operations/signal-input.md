...
`📥・信号输入` 是 Manager 提交原始交易信息的唯一入口。当前运行链路完成可靠收件、
附件验证、数据库保存、`SIGNAL_PARSE` 和 `✅・信号审核`；只有 Manager 明确点击
确认发布后才会向会员频道发卡。
...
- 文字加图片；
- Discord 转发消息中的正文与图片附件。

单张附件上限由 `MAX_ATTACHMENT_BYTES` 控制，默认 10 MB。真实文件签名是最终格式；当
Discord 转换图片后出现 `.webp` 文件名但 MIME/实际内容为 PNG 时，系统按真实签名安全归一化。
真实签名必须至少匹配扩展名或 MIME 其中一项；两项都不匹配或内容不是
PNG/JPEG/WEBP 时仍会拒绝。
...
1. 用无 `Manager` Role 的账号确认无法看到 `📥・信号输入`。
...
6. 转发一条含正文和图片的 Discord 消息，确认转发快照被保存并解析。
7. 确认 `✅・信号审核` 只出现一张对应审核卡片，会员信号频道没有新卡片。
