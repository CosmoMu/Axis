...
Swing / LEAPS 的「编辑必填项」使用分区向导：先用下拉菜单选择新订单、加仓阶段、更新、TP、
SL、平仓、取消或滚仓，再分别编辑合约、价格 / 仓位、正股结构、目标 / 逻辑。每个输入框只允许
填写一个数据；Category、Mentor、关联订单、订单操作、Call / Put 和可用 Expiry 优先使用下拉
菜单。LEAPS 的「操作后总持仓」下拉为 optional：新入场留空默认 `1/8`，更新已有订单留空保留
当前持仓，SL / 平仓 / 取消仍自动归零。Swing 保持其独立 Simple Tracked Swing 规则。发布阻塞
信息必须显示中文字段名并逐项列出，不得只返回内部字段代码。
