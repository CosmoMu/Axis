...
填写一个数据；Category、Mentor、关联订单、订单操作、Call / Put、可用 Expiry 和操作后总持仓
优先使用下拉菜单。发布阻塞信息必须显示中文字段名并逐项列出，不得只返回内部字段代码。
...
- High / Low Watermark、固定 TP1–TP41、Fast Momentum Reversal、Tracking Protection、
